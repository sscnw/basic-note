...
- 是一个高性能的HTTP和反向代理服务器，也是一个IMAP/POP3/SMTP服务器。**在高连接并发的情况下，Nginx是Apache服务器不错的替代品**:
- 特点：轻量级，占有内存少，并发能力强。
- 服务器：Nginx作为负载均衡服务器：既可以在内部直接支持Rails和PHP程序对外进行服务，也可以支持作为HTTP代理服务器对外进行服务。Nginx采用C进行编写，系统资源开销和CPU使用效率逗比Perlbal好很多
- 代码：Nginx有自己的函数库，除了zlib、PCRE和OpenSSL之外，标准模块只使用系统C库函数。
...
- 事件模型：Nginx支持如下处理连接的方法（I/O复用方法），这些方法可以通过`use`指令指定。
  ...
  - **eventport** - 高效的方法，使用于 Solaris 10. 为了防止出现内核崩溃的问题， 有必要安装 [这个](https://www.w3cschool.cn/nginxsysc/vdkcxy.html) 安全补丁。
...
- $http_cookie 
- $limit_rate 
- $[request](http://www.php100.com/tags.php/request/)_body_file 
- $remote_addr
- $remote_port 
- $remote_user 
- $request_filename
...
- Nginx的Rewrite规则与Apache几乎完全一致，所不同的是最后的flag标记，举例如下：
...
- 实践是最好的老师，通过实践来对其配置做更多的了解，后续会慢慢更新哒！